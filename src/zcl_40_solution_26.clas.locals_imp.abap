...
"!Abstrakte Superklasse für Klassen: &nbsp                             " Übung 26
...
CLASS lcl_flight DEFINITION ABSTRACT.
  ...
  PUBLIC SECTION.
      ...
      ALIASES get_output for lif_output~get_output.                 " Übung 22 / 7
  ...
  PROTECTED SECTION.
    ...
    METHODS: get_description                                                             " Ubung 22 / 1
...
CLASS lcl_passenger_flight DEFINITION INHERITING FROM lcl_flight.
  ...
  PROTECTED SECTION.
     ...
     METHODS get_description REDEFINITION.                                   " Übung 22 / 3
...
*        VALUE(r_result) TYPE string_table.
  ...
  PROTECTED SECTION.
       ...
       METHODS get_description REDEFINITION.                                    " Übung 22 / 4
...
"! Flight Carrier -                                                                 " Übung 26
"! A factory logic ensures that there is only instance for the same carrier ID
CLASS lcl_carrier DEFINITION CREATE PRIVATE.                            " Übung 23  Privat machen
  ...
  PUBLIC SECTION.
    ...
    TYPES: tt_carriers TYPE STANDARD TABLE OF REF TO lcl_carrier WITH DEFAULT KEY.    " Übung 23 Tabelle mit Quickfixes erstellen

"! Factory method - returns an instance of this class.                                       " Übung 26
    ...
    CLASS-METHODS get_instance                                           " Übung 23 / 1
      IMPORTING
        i_carrier_id TYPE /dmo/carrier_id
      RETURNING
        value(r_result) TYPE REF TO lcl_carrier
      RAISING                                                            " Übung 23 nach Quickfixes
*        cx_abap_auth_check_exception
*        cx_abap_invalid_value.                                          " Übung 25 dann zur implentierung
      ZCX_40_failed.
...
* Abap Docu Übung 26
    ...
    "! Suche nach einem <strong>Frachflug</strong>zwischen zwei Flughäfen die    * Übung 26
    "! <ul>
    "! <li>Liegt an oder nach einem bestimmten Datum</li>
...
CLASS lcl_carrier IMPLEMENTATION.
  ...
  SELECT SINGLE                                                                " Übung 23 vom Constructor
    FROM /LRN/carrier
...
*              EXPORTING                                                       " Übung 25 kommentieren
*              value = conv #(  i_carrier_id ).

       RAISE EXCEPTION TYPE zcx_40_failed                                      " Übung 25 einfügen
        EXPORTING
...
*         previous   =
    ...
    IF sy-subrc <> 0.                                                 " <> Keine Berechtigung  Anzeige = Anzeige berechtigt

*      RAISE EXCEPTION TYPE cx_abap_auth_check_exception                        " Übung 25 Kpmmentiern
*      EXPORTING                                                                " Übung 24
*                   textid =
*                     cx_abap_auth_check_exception=>missing_authorization..

        RAISE EXCEPTION TYPE zcx_40_failed                                      " Übung 25 einfügen                                                                         " Übung25
        EXPORTING
...
*   ENDIF.
   ...
   r_result = instances[ table_line->carrier_id = i_carrier_id  ].
    ...
    CATCH cx_sy_itab_line_not_found.                                               " Übung 23
...
*               ).
   ...
   r_result->currency_code = details-currency_code.                                " Übung 23
    ...
    APPEND r_result TO instances.                                                  " Übung 23 nach Tabelle
